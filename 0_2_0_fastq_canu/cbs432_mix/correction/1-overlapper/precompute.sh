...
if [ $jobid -eq 67 ] ; then
  rge="-b 594001 -e 603000"
  job="000067"
fi

if [ $jobid -eq 68 ] ; then
  rge="-b 603001 -e 612000"
  job="000068"
fi

if [ $jobid -eq 69 ] ; then
  rge="-b 612001 -e 621000"
  job="000069"
fi

if [ $jobid -eq 70 ] ; then
  rge="-b 621001 -e 630000"
  job="000070"
fi

if [ $jobid -eq 71 ] ; then
  rge="-b 630001 -e 633487"
  job="000071"
fi
...
/data/apps/java/jdk1.8.0_74/bin/java -d64 -server -Xmx6144m \
  ...
  --min-olap-length 500 \
  --num-threads 16 \
  -f  ../../0-mercounts/cbs432.ms16.frequentMers.ignore.gz  \
  -p  ./$job.input.fasta  \
  -q  .  \
&& \
